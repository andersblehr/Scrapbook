...
%%% Last Modified On: Sun Mar 28 06:37:44 1993
...
\item[Reference point:] ``{\em\ldots decide the position\ldots
  ...
  of the image has been as reference point, having coordinates
  $(0,0)$.
...
\item[Resolution:] ``{\em Preferably, the image should have a
  ...
  pixels with 64 gray levels.  In addition, due to limitations of the
  video camera which has been available, it was not possible to get
  close-ups of the monkey's eye that filled the entire field of view.
  Thus, these close-ups had to be made by shaving away the
  uninteresting portions of the images, leaving test images having an
  even lower resolution of approximately $100\times 100$ pixels.
  Lastly, the darkest gray level in the images supplied by the
  frame-grabber is 15, thus reducing the actual number of gray levels
  to 48.
\end{description}
Particularly the relatively low resolution of the test images has
put limitations on the quality of the tests that the algorithm has
...
\begin{figure}[tb]
  ...
  \makebox[0.45\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbl01.eps}}
  \makebox[0.4\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbr01.eps}}\\
  \hspace*{0.28\textwidth}(a)\hspace*{0.38\textwidth}(b)
  \paragraph{}
  \makebox[0.425\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbl02.eps}}
  \makebox[0.4\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbr02.eps}}\\
  \hspace*{0.28\textwidth}(c)\hspace*{0.38\textwidth}(d)
  \paragraph{}
  \makebox[0.425\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbl05.eps}}
  \makebox[0.4\textwidth][r]{
    \epsfbox{c:/user/blehr/fag/diplom/rapport/figurer/eps/monkbr03.eps}}\\
  \hspace*{0.28\textwidth}(e)\hspace*{0.38\textwidth}(f)
  \caption{\label{fig:testimages}The test images used while developing
    and testing {\octopus}.}
\end{figure}

The test images used while developing and testing {\octopus} are shown
in Fig.~\ref{fig:testimages}.  Note that the image in
Fig.~\ref{fig:compare}(a) is identical to image (e) above.  As can be
seen, the quality of the test images is not very good.  Note in
particular the strong reflections present in the images, and their
``flat'' appearance.
...
a number of test images, and the results obtained are evaluated with
respect to accuracy and time consumption.  Some known problems and
uncertainty factors are described in Section~\ref{algo:problem}, and
in Section~\ref{algo:future}, the most important aspects pertaining to
