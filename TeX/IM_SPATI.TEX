...
%%% Last Modified On: Tue Mar 16 02:36:18 1993
...
pixel, applying the operator at each location $(x,y)$ in $f$.
...
\insertepswidth{mask}{\label{fig:mask}(a) Sub-area of image. (b) A
  $3\times 3$ mask with general coefficients.}{0.8}
...
corresponding pixel in $f$ and its 8 immediate neighbours ({\em
  8-neighbours\/}).  This is known as {\em neighbourhood averaging\/}
(see Section~\ref{image:noise:averaging}), and the effect is one of
{\em blurring\/} the original image.
...
perform a variety of useful image operations, among which are {\em
  noise reduction\/} (Section~\ref{image:noise}) and {\em feature
