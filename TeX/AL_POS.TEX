...
%%% Last Modified On: Mon Apr  5 15:17:03 1993
...
one iteration\footnote{In the current implementation, this number was
  set to 15.  However, it never happened that the iteration process
  had to be interrupted before a final position estimate had been
  obtained}.  If it should happen that the allowed number of
iterations has been carried out without having arrived at a final
overall estimate, the current estimate has to be returned.  Clearly,
the estimate thus returned would hardly satisfy the requirement of
...
vertical resolution $N$ of the $N\times N$ image.  Consequently, the
position determining part of the {\octopus} eye-tracking algorithm
